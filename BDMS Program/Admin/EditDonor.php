...
if ($conn->connect_error){
    ...
    // Retrive current details of Donor from database
    $stmt = "SELECT * FROM Donor WHERE Username = '$User_To_Edit'";
    ...
    if (isset($_POST["Confirm"])){
        ...
        (trim($DOB) == "") || (trim($MobileNumber) == "") || (trim($Disease) == "")){
            ...
            if (($row['Username'] == $User_To_Edit) || (mysqli_num_rows($result) == 0)){
                // Update Donor table in the database
                $stmt = $conn->prepare("UPDATE Donor SET Username=?, Name=?, Gender=?, DOB=?, PhoneNumber=?, Blood_Group=?, Medical_History = ?, Password=? WHERE Username=?");
                $stmt->bind_param("ssssissss", $Username, $FullName, $Gender, $DOB, $MobileNumber, $BloodGroup, $Disease, $Password, $User_To_Edit);
                $stmt->execute();

                // Update Donor History table in the database
                $stmt = $conn->prepare("UPDATE Donation_History SET Username=? WHERE Username=?");
                ...
                $_SESSION["Message"] = "Donor details updated successfully.";
                exit;
            }
            else{
...
<head>
    ...
    <link rel="stylesheet" href="AdminStyles/EditDonor.css">
</head>
<body style="margin: 0; padding-top: 40px; background-color: rgb(0, 120, 120);">
    ...
    <div class="Contents">
        ...
        <div class="EditDonorPage">
            <div style="width: 100%; border-radius: 10px;">
            <div class="EditDonorTitle Text">EDIT DONOR</div>
