...
if ($conn->connect_error){
    ...
    // Calculate total Units of blood
    $Total_Units = $A_Positive + $A_Negative + $B_Positive + $B_Negative + $O_Positive + $O_Negative + $AB_Positive + $AB_Negative;

    // Fetch total donors count from database
    ...
    // Fetch total requests and approved requests from database
    $stmt = "SELECT * FROM Blood_Request";
    $result = mysqli_query($conn,$stmt);
    $Request_Count = 0;
    $Approved_Requests = 0;
...
<head>
    ...
    <link rel="stylesheet" href="AdminStyles/AdminDashboard.css">
</head>
